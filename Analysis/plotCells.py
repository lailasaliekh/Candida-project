...
def plotCells(file,add_director=True):
    ...
    cells=getCells(file)
    for cell in cells:
        cell.colour=RodShapedBacterium.colour_fn(cell.theta)# can be changed to nematic order orientation
        #print(cell.nematic_ori[0])
    dat=pd.read_csv(file,sep='\t')
    maxx=dat['pos_x'].max()+3
    minx=dat['pos_x'].min()-3
    maxy=dat['pos_y'].max()+3
    miny=dat['pos_y'].min()-3
    fcp.addAllCellsToPlot(cells,ax,ax_rng=maxx-minx,show_id=True,ec=cell.colour)
