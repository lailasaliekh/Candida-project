...
def main():
    ...
    parser.add_argument("files", nargs="+",
                        help="biofilm*.dat files (globs OK, multiple repeats OK)")
    parser.add_argument("--edge-threshold", type=float, default=2.0,
                        metavar="N",
                        help="Edge = cells within N × mean_radius of hull boundary "
                             "(default: 2.0)")
    ...
    # Parse growth factor from each path; group files by gf (for repeats)
    groups = defaultdict(list)   # gf → [path, ...]
    for p in paths:
        try:
            gf = parse_growth_factor(p)
    ...
    if not groups:
        print("No files with parseable growth factors."); sys.exit(1)

    gf_sorted = sorted(groups.keys())
    print(f"\nFound {len(gf_sorted)} growth factor(s): {gf_sorted}")
    ...
    for gf in gf_sorted:
        ...
        if not repeat_metrics:
            print(f"  gf={gf}: no valid data, skipping.")
            continue
        ...
        if n_rep > 1:
            keys = repeat_metrics[0].keys()
            std  = {k: np.std([m[k] for m in repeat_metrics]) for k in keys}
        else:
            std  = {k: 0.0 for k in repeat_metrics[0].keys()}
    ...
    # Filter gf_sorted to only those with data
    valid_gf = [gf for gf, _ in zip(gf_sorted, avg_metrics_list)]

    print_table(valid_gf, avg_metrics_list)

    save_path = None
    if args.save:
        save_path = Path(paths[0]).parent.parent.parent / "edge_dominance.png"
